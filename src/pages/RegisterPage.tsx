...
export function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Welcome to the sacred circle! Please log in.');
      navigate('/login');
    } catch (error) {
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-divine-burgundy/70 mt-2">
              Embark on your journey to holistic wellness
            </p>
          </div>
          ...
          <Card className="border-divine-rose-gold/20 shadow-xl bg-divine-ivory/95 backdrop-blur-sm">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                {/* Username Field */}
                <div>
                  <label htmlFor="username" className="block text-sm font-medium text-divine-burgundy mb-2">
                    Username
                  </label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-divine-burgundy/50 w-4 h-4" />
                    <Input
                      id="username"
                      type="text"
                      value={username}
                      onChange={(e) => setUsername(e.target.value)}
                      placeholder="Choose your goddess name"
                      required
                ...
                <div>
                  <label htmlFor="confirm-password" className="block text-sm font-medium text-divine-burgundy mb-2">
                    Confirm Password
                  </label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-divine-burgundy/50 w-4 h-4" />
                ...
                >
                  {isLoading ? 'Joining...' : 'Join the Sisterhood'}
                </Button>
              </form>
